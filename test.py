...
def main():
    ...
    parser.add_argument('--obj', type=str, default='real_front')
    parser.add_argument('--data_type', type=str, default='50_euro')
    parser.add_argument('--data_path', type=str, default='dataset/50_euro')
    parser.add_argument('--checkpoint_dir',
                        type=str,
                        default='./50_euro/real_front/seed_3514/real_front_2023-12-24-1147_model.pt')
    parser.add_argument("--grayscale", action='store_true', help='color or grayscale input image')
    parser.add_argument('--batch_size', type=int, default=16)
    parser.add_argument('--img_size', type=int, default=256)
    parser.add_argument('--seed', type=int, default=4420)
    parser.add_argument('--ratio', type=float, default=95)
    parser.add_argument('--k_value', type=int, nargs='+', default=[4, 8, 16, 32])
    args = parser.parse_args()
    args.save_dir = './results/' + args.obj + '/'
    ...
    plt.legend(loc="lower right")
    save_dir = args.save_dir + '/' + f'seed_{args.seed}_test' + '/' + 'pictures_{:.4f}'.format(threshold)
...
def plot_fig(args, test_img, recon_imgs, scores, gts, threshold, save_dir):
    ...
    for i in range(num):
        ...
        fig_img, ax_img = plt.subplots(1, 6, figsize=(20, 6))
        ...
        ax_img[1].set_title('Reconst')
        ax_img[2].imshow(gt, cmap='gray')
        ax_img[2].set_title('GroundTruth')
        ax = ax_img[3].imshow(heat_map, cmap='jet')
        ...
        ax_img[3].set_title(f'Max Heat Map:{heat_map.max()}')
        ax_img[4].imshow(mask, cmap='gray')
        ax_img[4].set_title('Predicted mask')
        ax_img[5].imshow(vis_img)
        ax_img[5].set_title('Segmentation result')
        ...
        fig_img.savefig(os.path.join(save_dir, f"{args.obj}_{i}_png"), dpi=100)
        plt.close()
